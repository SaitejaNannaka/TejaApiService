...
async function sendApiDataToServer(apiRequestData) {
    try {
        const response = await fetch('http://localhost:3000/api/storeApiRequest/Api_Validation_Automated_TestCases/', {
            method: 'POST',
            headers: {
...
// Utility function to get all keys from an object recursively
...
//            const originalRequestBody = body ? JSON.parse(body) : {};
            const existingKeys = getAllKeysFromObject(originalRequestBody);

            if (removeKeys) {
                const keysToRemove = removeKeys.split(',').map(key => key.trim());
                const validRemoveKeys = keysToRemove.filter(key => existingKeys.includes(key));
                for (const key of validRemoveKeys) {
            ...
            if (invalidKeys) {
                ...
                const validInvalidKeys = keysToInvalidate.filter(key => existingKeys.includes(key));
                for (const key of validInvalidKeys) {
                    const modifiedBody = generateModifiedRequestBody(originalRequestBody, [], [key], false, customInvalidValue);
                    selectedTestCases.push([`Invalid Key "${key}"`, expectedStatus2, modifiedBody]);
                }
            }

            if (removeKeyValuePairs) {
                const keysToRemove = removeKeyValuePairs.split(',').map(key => key.trim());
                const validRemoveKeyValuePairs = keysToRemove.filter(key => existingKeys.includes(key));
                for (const key of validRemoveKeyValuePairs) {
                    const modifiedBody = generateModifiedRequestBody(originalRequestBody, [key], [], true);
                    selectedTestCases.push([`Remove Key-Value Pair for Key "${key}"`, expectedStatus3, modifiedBody]);
                }
            }
